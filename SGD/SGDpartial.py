...
for train_index, test_index in kf.split(one_hot_encoding_df):
    ...
    X_test = scaler.transform(test_X)  # apply same transformation to test dat
    
    est = make_pipeline(StandardScaler(), SGDClassifier(loss="hinge", penalty="l2", max_iter=100))
    est.fit(train_X,train_y)
    test_predict = est.predict(test_X)
